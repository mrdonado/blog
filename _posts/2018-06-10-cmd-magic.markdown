...
- `wget -c -t 0 --timeout=60 --waitretry=60 http://example.com/file.mkv` download the file using the partially downloaded file (resume `-c`) retrying ad infinitum (`-t 0`) with 60s timeout and 60s wait between attempts
